define(["require", "exports", "./error"], function (require, exports, error_1) {
    "use strict";
    /**
    ...
    var ErrorDispatcher = (function () {
        function ErrorDispatcher() {
            ...
            this.randomErrorButtonDom.on("click", function (e) {
                _this.addRandomError();
            });
        }
        ErrorDispatcher.prototype.addRandomError = function () {
            var params = errors[Math.floor(Math.random() * errors.length)];
            this.addError(new error_1.RoadError(params.code, params.name, params.points));
        };
        ErrorDispatcher.prototype.addError = function (error) {
        ...
        ErrorDispatcher.prototype.dispatchCode = function (code) {
            var errorId = "-1";
            for (var i in errors) {
            ...
            this.addError(new error_1.RoadError(error.code, error.name, error.points));
        };
        ErrorDispatcher.prototype.updatePointsAndPercentage = function () {
            var totalPoints = 0;
            var percentage = 100;
            for (var i in this.errors) {
                totalPoints += this.errors[i].getPoints();
            }
            var failedPercent = (totalPoints * 100) / 5;
    ...
    exports.ErrorDispatcher = ErrorDispatcher;
});
